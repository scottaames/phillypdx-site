<template>
  <div>
    <v-fab-transition v-if="!this.$vuetify.breakpoint.mobile" mode="out-in">
      <v-btn
        v-if="delayedShow"
        color="orangish"
        class="cart-button"
    ...
    <v-fab-transition v-else mode="out-in">
      <v-btn
        :key="activeFab.icon"
      ...
      >
        <v-badge
          ...
          right
        >
          <v-icon color="white">{{ activeFab.icon }}</v-icon>
        </v-badge>
      </v-btn>
    </v-fab-transition>
    ...
    <v-navigation-drawer
      ...
      width="400"
      style="z-index:999;"
      :hide-overlay="!this.$vuetify.breakpoint.mobile"
      light
    >
      <v-list-item two-line class="white">
        <v-list-item-title class="text-h5 text-center font-weight-medium">
          ...
          <span :class="this.$vuetify.breakpoint.mobile ? '' : 'mr-6'"
            >Order Cart</span
          >
        </v-list-item-title>
      </v-list-item>
...
export default {
  ...
  computed: {
    activeFab() {
      if (!this.drawer) {
        return { color: 'orangish', icon: 'mdi-cart-outline' }
      }
      return { color: 'red', icon: 'mdi-close-thick' }
    },
    getBadgeColor() {
